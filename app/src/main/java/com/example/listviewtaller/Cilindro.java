...
public class Cilindro extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        DatoRadio = findViewById(R.id.lblDatoCilindroRadio);
        DatoAltura = findViewById(R.id.lblDatoCilindroAltura);
        txtRadio = findViewById(R.id.txtCalcularCilindroRadio);
        txtAltura = findViewById(R.id.txtCalcularCilindroAltura);
    }
    ...
    public void guardar(View v){
        ...
        operacion = getResources().getString(R.string.guardar_VolumenCilindro);

        Operaciones Op;
        if(validar()){
            dato = txtRadio.getText().toString() +": "+ DatoRadio.getText().toString()+" - "+txtAltura.getText().toString() +": "+ DatoAltura.getText().toString();

            result = Math.PI * Math.pow(Integer.parseInt(DatoRadio.getText().toString()),2) * Integer.parseInt(DatoAltura.getText().toString());
